...
const Post = () => {
  ...
  const fetchLikesForAPost = async () => {
    const response = await fetchData.getLikesForAPost(
      user?.accessToken,
      postId
    );
  ...
  const fetchComments = async () => {
    try {
      await fetchData
        .getCommentsForAPost(user?.accessToken, postId)
        .then((respo) => {
  ...
  const getSavedPost = async () => {
    const response = await fetchData.getSavedPost(
      user?.accessToken,
      user?.user?._id
    );
  ...
  const handleSave = async () => {
    try {
      const response = await fetchData.handleSave(
        user?.accessToken,
        user?.user?._id,
        postId
      );
  ...
  const handleLike = async () => {
    try {
      const response = await fetchData.handleLike(
        user?.accessToken,
        user?.user?._id,
        postId
      );
  ...
  const handleAddComment = async () => {
    try {
      if (commentValue.trim() !== "") {
        await fetchData
          .addComment(user?.accessToken, postId, user?.user?._id, commentValue)
          .then((respo) => {
            setCommentVlaue("");
            fetchComments();
          });
  ...
  const handleCommentDelete = async (commentId) => {
    try {
      await fetchData
        .deleteComment(user?.accessToken, commentId)
        .then((respo) => {
          fetchComments();
        });
    } catch (error) {
  ...
  return (
    <div className="flex ">
      <div className="w-2/3">
        {isPostLoading ? (
          <Stack height={"500px"} width={"600px"} padding={"15px"}>
          ...
          <div className="">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-5">
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden">
                  <img src={postData?.userId?.profileImg} alt="" />
                </div>
                <div>
                  <div className="text-sm flex gap-1">
                    <Link to={`/${postData?.userId?.username}`}>
                      {postData?.userId?.fullName}
                    </Link>
                    <div className="text-gray-600 text-sm flex items-center gap-1">
                      <span className="h-1 w-1 bg-gray-600 rounded-full aspect-square"></span>
                      6hr ago
                    </div>
                  </div>
                ...
                {dropdownOpen && (
                  <div className="absolute border border-gray-700 right-0 z-10 bg-black divide-y divide-gray-100 rounded-lg shadow w-44">
                    <ul className="py-2 text-sm text-gray-300 flex flex-col">
                      <Link
                        className="py-2 text-center hover:bg-gray-900"
                        to={`/${postData?.userId?.username}`}
                      >
                        Visit Profile
                      </Link>
            ...
            <div className="relative w-full rounded-xl border my-2 border-gray-950 overflow-hidden">
              {/* Blurred background using a pseudo-element */}
              <div
                className="absolute inset-0 bg-cover bg-center blur-2xl"
                style={{ backgroundImage: `url(${postData.imgUrl})` }}
              ></div>
              ...
              <img
                src={postData.imgUrl}
                alt="Post"
                className="relative z-10 w-auto m-auto max-h-[435px]"
              />
            </div>
            ...
            <div className="flex justify-between px-1 pt-4 items-center">
              <div className="flex space-x-4 items-center">
                <div className="text-xl bg-gray-900 rounded-full px-4 py-1 text-gray-400 flex items-center">
                  {likedUsers.includes(user?.user?._id) ? (
                    <BiSolidUpvote onClick={handleLike} />
                  ) : (
                    <BiUpvote onClick={handleLike} />
                  )}
                  <span className="text-sm"> {totalLikes && totalLikes}</span>
                </div>
        ...
        <div>
          <div className="py-6 flex gap-4">
            ...
            <input
              ...
              className="bg-transparent rounded-full w-full focus:border-gray-700 focus:outline-none border-gray-800"
              value={commentValue}
              onChange={(e) => setCommentVlaue(e.target.value)}
            />
            <Button
            ...
            comments.map((c, i) => (
              <div key={i} className="flex gap-2 pt-3 ">
                ...
                <div className="w-full overflow-hidden">
                  ...
                  <div className="border-b border-gray-700 pb-3 max-w-full">
                    {c?.content}
                  </div>
                </div>
              </div>
